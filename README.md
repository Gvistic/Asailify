...
| 10 | `~notify add [Channel*/*User*/Role*] [Snowflake*]` | Adds a channel/user/role to the notification list, whenever a match is made via scan or event the bot will notify the channels, and DM's users in the list. Adding a role to the notification list will simply mention the role in a channel notification message. A role will not be notified if it's a scan, custom scan or multiple occurances of the same user have already been notified. | `~notify add channel #mods`, `~notify add user 127932752744939521`, `~notify add role @Staff`
